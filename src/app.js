...
function startRouteFrom(lat, lon, label) {
  ...
  showRouteHint(`Trasa piesza od: <b>${routeState.origin.label}</b> — kliknij punkt docelowy na mapie.`);
}
...
async function computeRoute(dest) {
  const o = routeState.origin;
  layers.route.clearLayers();
  showRouteHint("Wyznaczam trasę pieszą…");
  const url = `https://api.mapbox.com/directions/v5/mapbox/walking/` +
    `${o.lon},${o.lat};${dest.lon},${dest.lat}` +
  ...
  L.polyline(line, { color: "#7c1d6f", weight: 5, opacity: 0.85, interactive: false }).addTo(layers.route);
  L.polyline(line, { color: "#fff", weight: 9, opacity: 0.35, interactive: false }).addTo(layers.route).bringToBack();
  L.circleMarker([o.lat, o.lon], { radius: 6, color: "#fff", weight: 2, fillColor: "#0ca30c", fillOpacity: 1 })
    .addTo(layers.route).bindTooltip("Start: " + o.label);
  L.circleMarker([dest.lat, dest.lon], { radius: 6, color: "#fff", weight: 2, fillColor: "#d03b3b", fillOpacity: 1 })
    .addTo(layers.route).bindTooltip("Cel (wskazany ręcznie)");

  const meters = route.distance, walkMin = route.duration / 60;
  // porównanie z modelem demonstracyjnym (linia prosta × współczynnik)
  const straight = distM({ lat: o.lat, lon: o.lon }, dest);
  const modelMin = timeOneWayMin(straight);
  const km = meters >= 1000 ? (meters / 1000).toFixed(2).replace(".", ",") + " km" : Math.round(meters) + " m";

  L.popup({ maxWidth: 260, autoPan: true })
    .setLatLng(line[Math.floor(line.length / 2)])
    .setContent(
      `<b>Trasa piesza (Mapbox)</b><br>` +
      `dystans: <b>${km}</b><br>` +
      `czas dojścia: <b>${walkMin.toFixed(1).replace(".", ",")} min</b> <span style="color:#898781">(≈5 km/h)</span><br>` +
      `<span style="color:#898781">w linii prostej: ${Math.round(straight)} m · model demo: ${modelMin.toFixed(1).replace(".", ",")} min</span><br>` +
      `<a href="#" data-route-clear="1">Wyczyść trasę</a>`)
    .openOn(map);
  map.getContainer().style.cursor = "";
  routeState.active = false;
  showRouteHint(
    `Trasa: <b>${km}</b> · <b>${walkMin.toFixed(1).replace(".", ",")} min</b> pieszo ` +
    `<a href="#" id="route-clear-hint">wyczyść</a>`);
}
...
map.on("click", (e) => {
  if (routeState.active && routeState.origin) computeRoute({ lat: e.latlng.lat, lon: e.latlng.lng });
});
// bindowanie linku „Wyczyść trasę" w popupie wyniku
map.on("popupopen", (e) => {
  const el = e.popup.getElement();
  if (el) el.querySelectorAll("a[data-route-clear]").forEach((a) =>
...
function drawProps() {
  ...
  state.proposals.forEach((p, i) => {
    ...
    mk.bindPopup(
      ...
      `<a href="#" data-card="${p.id}">Karta wdrożenia →</a> · <a href="#" data-route="${p.id}">📏 trasa piesza</a> · <a href="#" data-del="${p.id}">usuń</a>`
    );
    mk.on("popupopen", (e) => bindCardLinks(e.popup.getElement()));
    mk.on("dragend", () => {
...
function bindCardLinks(el) {
  ...
  el.querySelectorAll("a[data-route]").forEach((a) =>
    a.addEventListener("click", (ev) => {
      ...
      if (src) startRouteFrom(src.lat, src.lon, src.name || src.host || id);
    }));
...
function initUI() {
  ...
  map.on("click", (e) => {
    if (routeState.active) return;   // tryb pomiaru trasy ma pierwszeństwo
    if (!addMode) return;
    state.proposals.push({
      id: "NOWY-" + String(state.proposals.length + 1).padStart(2, "0"),
