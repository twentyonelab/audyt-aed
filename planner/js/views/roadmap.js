 ...
 *
 * Dwa tryby jednego planu wdrożenia, przełączane w sub barze:
 *   • Kanban   — trzy kolumny faz (+ „Nieprzypisane") z drag & drop między
 *                fazami, paskiem mapy w kolorach faz i zatwierdzeniem kroku,
 *   • Oś czasu — Gantt na 24 miesiące (8 kwartałów) z przeciąganiem pasków,
 *                kamieniami milowymi i krzywą pokrycia w czasie.
 *
...
export async function render(root, ctx) {
  ...
  const view = state.ui.roadmapView === 'timeline' ? 'timeline' : 'kanban';
  const recommendations = state.recommendations || [];

  /* ---------------- Sub bar: [Kanban | Oś czasu] ---------------- */
...
function buildTimeline({ recommendations, phases, unassigned, nowAnalysis, afterPhase1, afterPhase2, afterPlan }) {
  const baseYear = Number(String(TODAY).slice(0, 4)) || new Date().getFullYear();

  const tasksCol = h('div', { class: 'gantt__tasks' });
  const axisCol = h('div', { style: { minWidth: '0' } });

  /* --- nagłówek: kolumna zadań 430 px + oś 24 miesięcy w 8 kwartałach --- */

  tasksCol.appendChild(
    h(
      'div',
      { class: 'gantt__row gantt__row--head' },
      h('span', { style: { flex: '1' }, text: 'Zadanie' }),
      h('span', { text: 'koszt' })
    )
  );
  ...
  axisCol.appendChild(
    h(
      'div',
      { class: 'gantt__quarters' },
      ...Array.from({ length: QUARTERS }, (_, i) =>
        h('div', {
          class: 'gantt__quarter',
          text: `Q${(i % 4) + 1} ${baseYear + Math.floor(i / 4)}`,
          title: `Miesiące ${fmtNum(i * MONTHS_PER_QUARTER + 1)}–${fmtNum((i + 1) * MONTHS_PER_QUARTER)}`,
        })
      )
    )
  );

  /* --- wiersze grupowane fazami --- */

  const groups = [
    ...PHASES.map((n) => ({
      phase: n,
      label: `${PHASE_META[n].label} — ${PHASE_META[n].title}`,
      months: PHASE_META[n].months,
      items: phases[n].items,
      cost: phases[n].cost,
    })),
  ...
  for (const group of groups) {
    tasksCol.appendChild(
      h(
        ...
        h('span', { style: { flex: '1' }, text: group.label }),
        h('span', { class: 'muted', style: { fontSize: '11px' }, text: group.months }),
        h('span', { class: 'num', text: fmtCost(group.cost) })
      )
    );
    axisCol.appendChild(h('div', { class: 'gantt__lane', style: { background: 'var(--section)' } }));

    for (const rec of group.items) {
      tasksCol.appendChild(
        h(
          'div',
          { class: 'gantt__row', title: `${rec.text} — ${recSource(rec)}` },
          h('span', {
          ...
          h('span', {
            class: 'muted',
            style: { fontSize: '11px', maxWidth: '150px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
            text: recSource(rec),
          }),
          h('span', { class: 'num', text: fmtCost(rec.cost) })
        )
      );
      axisCol.appendChild(ganttLane(rec, group.phase));
    }
  }

  /* --- kamienie milowe --- */

  const end1 = phaseEndMonth(phases[1].items, PHASE_END_FALLBACK[1]);
  const end2 = Math.max(end1, phaseEndMonth(phases[2].items, PHASE_END_FALLBACK[2]));
  const end3 = Math.max(end2, phaseEndMonth(phases[3].items, PHASE_END_FALLBACK[3]));

  const milestones = [
    { month: end1, label: 'Zgodność podstawowa', phase: 1, pct: afterPhase1.coveragePct },
    { month: end2, label: `Pokrycie ${fmtPct(afterPhase2.coveragePct, 0)}`, phase: 2, pct: afterPhase2.coveragePct },
    { month: end3, label: `Pokrycie ${fmtPct(afterPlan.coveragePct, 0)}`, phase: 3, pct: afterPlan.coveragePct },
  ];
  ...
  tasksCol.appendChild(
    h(
      'div',
      { class: 'gantt__row gantt__row--head' },
      h('span', { style: { flex: '1' }, text: 'Kamienie milowe' })
    )
  );
  axisCol.appendChild(
    h(
      'div',
      { class: 'gantt__milestones' },
      ...milestones.map((m) =>
        h(
          'div',
          {
            class: 'gantt__milestone',
            style: { left: `${clamp((m.month / MONTHS) * 100, 3, 97)}%`, whiteSpace: 'nowrap' },
            title:
              `${m.label} — koniec ${PHASE_META[m.phase].label}, miesiąc ${fmtNum(m.month)}; ` +
              `pokrycie ${fmtPct(m.pct, 1)}`,
          },
          m.label
        )
      )
  ...
  tasksCol.appendChild(
    h(
      ...
      {
        class: 'gantt__row',
        style: { height: `${CURVE.height}px`, alignItems: 'flex-start', paddingTop: '10px', display: 'block' },
      },
      h('span', { class: 'label-caps', text: 'Krzywa pokrycia' }),
      h('div', {
        class: 'note num',
        text: `teraz ${fmtPct(nowAnalysis.coveragePct, 0)} → po planie ${fmtPct(afterPlan.coveragePct, 0)}`,
      })
  ...
  axisCol.appendChild(
    coverageCurve([
      { month: 0, pct: nowAnalysis.coveragePct },
      { month: end1, pct: afterPhase1.coveragePct },
      { month: end2, pct: afterPhase2.coveragePct },
      { month: end3, pct: afterPlan.coveragePct },
    ])
  );
  ...
  return h(
    'div',
    {},
    h('div', { class: 'gantt' }, h('div', { class: 'gantt__grid' }, tasksCol, axisCol)),
    h('p', {
      class: 'note',
      style: { marginTop: '8px' },
      text:
        'Przeciągnij pasek, aby zmienić miesiąc startu; złap prawą krawędź, aby zmienić czas ' +
        `trwania. Terminy zaokrąglamy do pełnych miesięcy w zakresie 1–${fmtNum(MONTHS)}. ` +
        'Kamienie milowe i krzywa liczone są funkcją analyze() dla trzech scenariuszy: ' +
        'tylko istniejące punkty, istniejące + fazy 1–2, istniejące + całość planu.',
    })
  );
}

/** Jeden tor osi z paskiem zadania (albo notką dla pozycji bez fazy). */
function ganttLane(rec, phase) {
  const lane = h('div', { class: 'gantt__lane' });
  ...
  if (!phase) {
    lane.appendChild(
      h('span', {
        ...
        text: 'bez fazy — przypisz w trybie Kanban',
      })
    );
    return lane;
  }

  const base = barMonths(rec);
  const handle = h('div', { class: 'gantt__handle', title: 'Przeciągnij, aby zmienić czas trwania' });
  const bar = h(
    'div',
    {
      class: `gantt__bar gantt__bar--${phase}`,
      style: {
        left: `${leftPct(base.start)}%`,
        width: `${widthPct(base.length)}%`,
        paddingRight: '12px',
      },
      title: `${rec.text} — start: miesiąc ${fmtNum(base.start)}, czas trwania: ${fmtNum(base.length)} mies.`,
    },
    rec.text,
    handle
  );
  ...
  const beginDrag = (event, kind) => {
    ...
    const perMonth = (lane.clientWidth || 1) / MONTHS;
    const originX = event.clientX;
    const start = barMonths(rec);
    let next = { ...start };

    const apply = () => {
      bar.style.left = `${leftPct(next.start)}%`;
      bar.style.width = `${widthPct(next.length)}%`;
      bar.title = `${rec.text} — start: miesiąc ${fmtNum(next.start)}, czas trwania: ${fmtNum(next.length)} mies.`;
    };

    const onMove = (ev) => {
      const delta = Math.round((ev.clientX - originX) / perMonth);
      next =
        kind === 'move'
          ? { start: clamp(start.start + delta, 1, MONTHS - start.length + 1), length: start.length }
          : { start: start.start, length: clamp(start.length + delta, 1, MONTHS - start.start + 1) };
      apply();
    };
    ...
    const finish = () => {
      ...
      if (releasePointer === finish) releasePointer = null;
    };

    const onUp = async () => {
      finish();
      if (next.start === start.start && next.length === start.length) return;
      upsertRecommendation({ id: rec.id, startMonth: next.start, lengthMonths: next.length });
      await save();
      toast(
        `„${rec.text}": start w miesiącu ${fmtNum(next.start)}, czas trwania ${fmtNum(next.length)} mies.`
      );
    };
...
/**
 * Krzywa pokrycia jako inline SVG (polyline). Oś X skalowana do szerokości
 * toru (preserveAspectRatio="none"), dlatego podpisy procentów są elementami
 * HTML pozycjonowanymi procentowo — inaczej tekst byłby rozciągnięty.
 */
function coverageCurve(nodes) {
  const values = nodes.map((n) => n.pct);
  const lo = Math.max(0, Math.floor(Math.min(...values)) - CURVE.padPct);
  const hi = Math.min(100, Math.ceil(Math.max(...values)) + CURVE.padPct);
  const span = hi - lo || 1;
  const plot = CURVE.height - CURVE.top - CURVE.bottom;

  const xOf = (month) => clamp((month / MONTHS) * 100, 0, 100);
  const yOf = (pct) => CURVE.top + (1 - (pct - lo) / span) * plot;

  const holder = h('div', { class: 'gantt__curve' });
  const points = nodes.map((n) => `${xOf(n.month).toFixed(2)},${yOf(n.pct).toFixed(2)}`).join(' ');

  holder.innerHTML =
    `<svg viewBox="0 0 100 ${CURVE.height}" preserveAspectRatio="none" role="img" ` +
    `aria-label="Krzywa pokrycia w czasie">` +
    `<polyline points="${points}" fill="none" style="stroke:var(--bar)" stroke-width="1.6" ` +
    `stroke-linejoin="round" vector-effect="non-scaling-stroke"/></svg>`;

  for (const node of nodes) {
    const x = xOf(node.month);
    const y = yOf(node.pct);
    holder.appendChild(
      h('span', {
        style: {
          position: 'absolute',
          // lekkie dociągnięcie skrajnych węzłów, żeby znacznik nie wychodził
          // poza tor osi (linia i tak mieści się w jego środku)
          left: `${clamp(x, 0.4, 99.6)}%`,
          top: `${y}px`,
          width: '9px',
          height: '9px',
          marginLeft: '-4.5px',
